...
function release()
  ...
  local entityId = world.spawnNpc(mcontroller.position(), self.monster.species, self.monster.type, self.monster.level, nil, sb.jsonMerge(self.monster.params, {
    damageTeam = damageTeam.team,
    damageTeamType = damageTeam.type,
    aggressive = self.monster.aggressive
  }))
  if entityId then
    local position = world.callScriptedEntity(entityId, "findGroundPosition", world.entityPosition(entityId), -80, 80, false)
