...
function build(directory, config, parameters, level, seed)
  ...
  if config.fixedStemHues then
    if type(config.fixedStemHues) == "table" then parameters.stemHueShift = util.randomChoice(config.fixedStemHues)
    else parameters.stemHueShift = config.fixedStemHues end
  end
  if config.fixedFoliageHues then
    if type(config.fixedFoliageHues) == "table" then parameters.foliageHueShift = util.randomChoice(config.fixedFoliageHues)
