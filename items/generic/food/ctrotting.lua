function ageItem(baseItem, aging)
  if baseItem.parameters.timeToRot then
    ...
    if baseItem.parameters.timeToRot <= 0 then
      ...
      return {
        name = itemConfig.config.rottedItem or root.assetJson("/items/generic/food/ctrotting.config:rottedItem"),
...
function getRotTimeDescription(rotTime)
  local descList = root.assetJson("/items/generic/food/ctrotting.config:rotTimeDescriptions")
